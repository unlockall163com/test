...
////// check
//
//  new checjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjk 
...
Copyright (c) 1989-2000 Microsoft Corporation
Copyright (c) 1989-2000 Microsoft Corporation
Copyright (c) 1989-2000 Microsoft Corporatioiiiiiiiiiiiiiiiiiiiiiiiiiiiiiin
...

{
    ...
    if (!FatDeviceIsFatFsdo( IoGetCurrentIrpStackLocation(Irp)->DeviceObject))  {
        ...
        if ((NodeType(Fcb) == FAT_NTC_FCB) &&
            ...
            //
			DbgPrint("FatFsdRead: Pageing read\n");
    ...
    try {
        ...
        if ( FlagOn(IrpContext->MinorFunction, IRP_MN_COMPLETE) ) {
            ...
            DbgPrint("FatFsdRead: Calling FatCompleteMdl\n");
        ...
        if (IoGetRemainingStackSize() < OVERFLOW_READ_THRESHHOLD) {
            ...
            DbgPrint("FatFsdRead: Passing StackOverflowRead off\n");
...

{
	...
	{
		KdPrint(("FatCommonRead:: Obj=%x,OrgObj=%x,Wait=%X,Offset=%X,Length=%X,Irp->Flags=%X,AllocationSize=%X,FileSize=%X,ValidDataLength=%X,FileName=%wZ\n",
			FileObject,Irp->Tail.Overlay.OriginalFileObject,Wait,IrpSp->Parameters.Write.ByteOffset.LowPart,IrpSp->Parameters.Write.Length,Irp->Flags,
			FcbOrDcb->Header.AllocationSize.LowPart,FcbOrDcb->Header.FileSize.LowPart,FcbOrDcb->Header.ValidDataLength.LowPart,&FcbOrDcb->FullFileName));
        ...
        if ( TypeOfOpen == UserFileOpen) {
            ...
            if ( PagingIo ) {
                ...
                if (!ExAcquireResourceSharedLite( FcbOrDcb->Header.PagingIoResource,
                    ...
                    DbgPrint("Cannot acquire FcbOrDcb = %08lx shared without waiting\n", FcbOrDcb );
                ...
                if (!Wait && NonCachedIo) {
                    ...
                    if (!FatAcquireSharedFcb( IrpContext, FcbOrDcb )) {
                        ...
                        DbgPrint("FatCommonRead: Cannot acquire FcbOrDcb = %08lx shared without waiting\n",FcbOrDcb );
            ...
            if (!PagingIo) {
                ...
                if (Status != STATUS_SUCCESS) {
					...
					DbgPrint("FatCommonRead: FsRtlCheckOplock error\n");
                ...
                if (!PagingIo &&
					...
					DbgPrint("FatCommonRead: FsRtlCheckLockForReadAccess error\n");
            ...
            if (StartingVbo >= FileSize) {
                ...
                DbgPrint("FatCommonRead: End of File\n");
...

            ...
            if ( NonCachedIo ) {
                ...
                DbgPrint("FatCommonRead: Non cached read.\n");
...

                ...
                if (!FlagOn(IrpContext->MinorFunction, IRP_MN_MDL)) {
                    ...
                    if (!CcCopyRead( FileObject,
                        ...
                        DbgPrint("Cached Read could not wait\n");
